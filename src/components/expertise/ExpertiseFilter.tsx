...
const ExpertiseFilter = ({
  ...
  return <div ref={ref} className={cn("max-w-3xl mx-auto mb-10", inView ? "opacity-100" : "opacity-0")} style={{
    ...
    transitionDelay: '200ms'
      ...
      <div className="flex flex-col items-center justify-center">
        ...
        if (value) onSelectCategory(value as ExpertiseCategory);
          ...
          {expertiseCategories.map(category => <ToggleGroupItem key={category.id} value={category.id} variant="outline" className="">
              {category.label}
            </ToggleGroupItem>)}
        </ToggleGroup>
      </div>
